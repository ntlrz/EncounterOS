...
GREEN   = QColor(50,200,100)
RED     = QColor(200,50,50)
YELLOW  = QColor(255,255,100)
GREY    = QColor(200,200,200)

# -------- Theme manager (overlay version) --------
class ThemeManager:
    """
    Overlay uses:
      - right_column
      - dialog_box
    Reads theme name from config.json and loads themes/<name>/theme.json for grid.
    """
    def __init__(self):
        self.grid = {
...
class Overlay(QWidget):
    ...
    def paintEvent(self, ev):
        ...
        if combat and full_party:
            ...
            p.setPen(WHITE); p.drawText(20, 28, f"Turn {turn_idx+1}/{len(full_party)}: {active_name}")
        ...
        for i, m in enumerate(render_party):
            ...
            # card bg
            p.fillRect(card, BLACK)
            pen = QPen(YELLOW if is_active else GREY); pen.setWidth(3); p.setPen(pen); p.drawRect(card)

            # portrait
            px = card.x()+10; py = card.y() + (box_h - ICON_SIZE.height())//2
            frame = QRect(px-2, py-2, ICON_SIZE.width()+4, ICON_SIZE.height()+4)
            p.fillRect(frame, QColor(0,0,0,120)); p.setPen(GREY); p.drawRect(frame)

            icon = getattr(self, "_cache_icons", {}).get(m["name"])
            if icon is None:
                icon = load_icon(m.get("icon","")); self._cache_icons[m["name"]] = icon
            if icon: p.drawPixmap(px, py, icon)

            # name + HP / status
            content_x = px + ICON_SIZE.width() + 12
            p.setPen(WHITE); p.setFont(self.font); p.drawText(content_x, card.y()+20, m["name"])
            ...
            if not m.get("isEnemy", False):
                ...
                p.fillRect(QRect(bar_x,bar_y,bar_w,bar_h), RED)
                p.fillRect(QRect(bar_x,bar_y,int(bar_w*ratio),bar_h), GREEN)

                # status badges on right side of bar
                ...
                for key in statuses[:4]:
                    ...
                    if pix:
                        ...
                        p.setPen(BLACK); p.setBrush(QBrush(GREY)); p.drawEllipse(sx, sy, 2*r, 2*r)
                        p.setPen(BLACK); p.setFont(self.font_small)
                        letter = key[:1].upper()
                        p.drawText(QRect(sx, sy, 2*r, 2*r), Qt.AlignCenter, letter)
                    sx -= STATUS_ICON_SZ.width() + 4

                p.setPen(WHITE); p.drawText(bar_x+6, card.y()+64, f"{curhp}/{maxhp} HP")
            else:
                if combat:
                    maxhp=max(1,int(m.get("maxHP",1))); curhp=max(0,min(int(m.get("currentHP",0)),maxhp))
                    ...
                    p.setPen(WHITE); p.drawText(content_x, card.y()+54, f"Status: {status}")

            # active badge
            if is_active:
                badge = QRect(card.right()-28, card.y()+8, 20, 20)
                p.fillRect(badge, YELLOW); p.setPen(BLACK); p.drawRect(badge)
        ...
        if not combat and self.dialog:
            ...
            p.fillRect(dlg, BLACK); p.setPen(GREY); p.drawRect(dlg)
            p.setPen(WHITE); p.setFont(self.dialog_font)
